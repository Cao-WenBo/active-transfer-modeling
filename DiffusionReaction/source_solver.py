...
class SourcePINN:
    ...
    def sample(self, seed):
        d = sample_training_data(self.case, seed)
        for k in ("X_bc", "s_bc", "X_r", "forcing_r"):
    ...
    def residual(self, theta):
        ...
        rb = bc[:, :1] - self.s_bc
        u, ut, uxx = r.split(1, 1)
        rp = ut - 0.01 * uxx - 0.01 * u.square() - self.forcing_r
        n = len(rb) + len(rp)
        return torch.cat([(n / len(rb)) ** 0.5 * rb, (n / len(rp)) ** 0.5 * rp])
    ...
    def save(self, path):
        ...
        core = {
            ...
            "derivatives": "explicit_minimal_[s,s_t,s_xx]",
