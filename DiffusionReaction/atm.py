...
def ensure_source(seed):
    ...
    if core_path.exists():
        ...
        if (
            core.get("derivatives")
            in (
                "explicit_batched_[s,s_x,s_t,s_xx]",
                "explicit_minimal_[s,s_t,s_xx]",
            )
...
def distance(a, b):
    delta = np.asarray(a["u_sensor"], np.float64) - np.asarray(
        b["u_sensor"], np.float64
    )
...
def main():
    ...
    result = {
        "config": {
            ...
            "coordinate_derivatives": "explicit_minimal_[s,s_t,s_xx]",
