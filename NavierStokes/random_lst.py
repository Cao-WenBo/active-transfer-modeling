"""Three-source single-source LST robustness experiment for Navier-Stokes."""
...
import torch
import config
from atm import build_solver, condition, distance_fields, initial_condition
from data_utils import generate_case, set_seed

SOURCE_SEEDS = [703, 726, 947]
...
def main():
    ...
    for source_seed in SOURCE_SEEDS:
        ...
        source_meta[str(source_seed)] = {
            ...
            "basis_reused": bool(solver.basis_reused),
            "cache_reused": bool(solver.cache_reused),
        }
        for index, target_seed in enumerate(TARGET_SEEDS, 1):
            ...
            result.update(
                ...
                distance=distance_fields(source_condition, condition(case)),
            )
            rows.append(result)
    ...
    save(
        RESULT,
        {
            "protocol": {
                ...
                "rank": config.RANK,
                "transfer_dtype": "strict_float32_no_TF32",
                "lambda_IC": config.LAMBDA_IC,
                "mu0": config.LM_INITIAL_MU,
                "GN_updates": config.GN_ITERS,
